...
<head>
    ...
    <link rel="stylesheet" href="../CSS/bandejaReportes.css">
</head>
...
<main class="reportes-container">
    ...
    <div id="bandeja" class="tab-content active">
        ...
        <!-- Filtros -->
        <div class="filtros-reportes-institucional">
            <input type="text" id="filtroBusquedaBandeja" placeholder="Buscar por ID, descripción...">
            <select id="filtroTipoBandeja">
            ...
            <select id="filtroGravedadBandeja">
            ...
            <button id="btnLimpiarFiltrosBandeja" class="btn-limpiar">Limpiar</button>
        </div>

        <div class="reportes-lista" id="bandejaLista">
            ...
            <?php else: ?>
                ...
                <div class="reporte-card" 
                     ...
                     data-gravedad="<?php echo $r['gravedad']; ?>"
    ...
    <div id="asignacion" class="tab-content">
        ...
        <div class="filtros-reportes-institucional">
            <input type="text" id="filtroBusquedaAsignacion" placeholder="Buscar por ID...">
            <select id="filtroTipoAsignacion">
                <option value="">Todos los tipos</option>
                <?php
                $tipos->data_seek(0);
                while($t = $tipos->fetch_assoc()){
            ...
            <button id="btnLimpiarFiltrosAsignacion" class="btn-limpiar">Limpiar</button>
        </div>
        <div class="reportes-lista" id="asignacionLista">
            <?php foreach($reportes as $r): ?>
            <div class="reporte-card" data-id="<?php echo $r['id']; ?>" data-tipo="<?php echo htmlspecialchars($r['tipo']); ?>">
                <div>
                    <h4>Reporte #<?php echo $r['id']; ?></h4>
                    <p>Tipo: <?php echo htmlspecialchars($r['tipo']); ?></p>
                </div>
    ...
    <!-- Actualizar estado, Comentarios, Cerrar caso (sin cambios) -->
    <div id="estado" class="tab-content">
        <h3>Actualizar estado del reporte</h3>
        <div class="input-group"><label>Seleccione reporte</label><select id="selectReporteEstado"><?php foreach($reportes as $r): ?><option value="<?php echo $r['id']; ?>">Reporte #<?php echo $r['id']; ?></option><?php endforeach; ?></select></div>
        <div class="input-group"><label>Nuevo estado</label><select id="nuevoEstadoReporte"><option value="Pendiente">Pendiente</option><option value="En Proceso">En proceso</option><option value="Resuelto">Resuelto</option></select></div>
        <button class="btn-guardar" id="btnActualizarEstado">Actualizar estado</button>
    </div>

    <div id="comentarios" class="tab-content">
        <h3>Agregar comentario</h3>
        <div class="input-group"><label>Seleccione reporte</label><select id="selectReporteComentario"><?php foreach($reportes as $r): ?><option value="<?php echo $r['id']; ?>">Reporte #<?php echo $r['id']; ?></option><?php endforeach; ?></select></div>
        <div class="input-group"><label>Comentario</label><textarea id="textoComentario" rows="4"></textarea></div>
        <button class="btn-guardar" id="btnGuardarComentario">Guardar comentario</button>
    </div>

    <div id="cerrar" class="tab-content">
        <h3>Cerrar caso</h3>
        <div class="input-group"><label>Seleccione reporte</label><select id="selectReporteCerrar"><?php foreach($reportes as $r): ?><option value="<?php echo $r['id']; ?>">Reporte #<?php echo $r['id']; ?></option><?php endforeach; ?></select></div>
...
<div id="modalDetalleBandeja" class="modal-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); justify-content: center; align-items: center; z-index: 1000;">
    <div class="modal-content" style="background: white; padding: 25px; border-radius: 16px; width: 500px; max-width: 90%; position: relative;">
        ...
        <div id="modalImagenContainer" style="margin-top: 15px; text-align: center;">
            <img id="modalImagen" src="" alt="Evidencia" style="max-width: 100%; border-radius: 12px; display: none;">
